...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package eglot
    ...
    :config
    (add-to-list 'eglot-stay-out-of 'flymake) ;; eglot must not start flymake automatically
...
Dive into the AI era with ~gptel~ and an ~ollama~ server running locally!

Setup of ~ollama~ with the ~llama3~ model:

#+BEGIN_SRC sh
  curl -fsSL https://ollama.com/install.sh | sh # install ollama
  ollama pull llama3                            # pull "llama3" from ollama models library
#+END_SRC
